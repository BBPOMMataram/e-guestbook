...
import { fetchItems, setItem, toggleFormPemDistribusiModal } from "../../services/pem-distribusi";
import FormModal from "./modals/pem-distribusi";
...
export default function PemDistribusi() {
    ...
    const fetchUrl = `/pem-distribusi?value_per_page=${valuePerPage}&page=${items?.current_page}`
    useEffect(() => {
    ...
    const getItem = async (id) => {
        const item = await axios.get(`/pem-distribusi/${id}`, {
            headers: {
    ...
    const removeItem = async (id) => {
        await axios.delete(`/pem-distribusi/${id}`, {
            headers: {
                'Authorization': 'Bearer ' + accessToken
            }
        })
    ...
    const btnNewHandler = () => {
        ...
        setModalFormTitle("Form Pemeriksaan Distribusi baru")
        setEditMode(false)
    }

    const btnEditHandler = async (id) => {
        setModalFormTitle("Form edit Pemeriksaan Distribusi")
    ...
    return (
        <section className="my-4">
            <h2 className="text-xl flex mb-2"><ArrowRightIcon width={20} className="mr-2" /> Pemeriksaan Distribusi</h2>
